...
int		main(int ac, char **av)
{
	...
	// Lien add in for the case of ./aout ab)
	int port = atoi(av[1]);
	...
	while (1)
	{
		...
		for (int fd = 0; fd <= max_fd; fd++)
		{
			...
			if (fd == sockfd)
			{
				...
				// Lien add in for the case of System call failures)
				if (client_fd < 0)
                    			fatal_error();
				if (client_fd >= 0)
				{
			...
			else
			{
				...
				// Lien add in for the case of Memory allocation failures)
				if (msgs[fd] == NULL) // memory allocation failure
                    			fatal_error();
				send_msg(fd);
			}
		}
